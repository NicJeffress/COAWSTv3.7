...
!
     ...
     &                          SEDBED(ng) % bed_frac(:,:,:,i))
              IF (FoundError(status, nf90_noerr, 2037, MyFile)) THEN
                IF (Master) THEN
...
!
     ...
     &                          SEDBED(ng) % bed_mass(:,:,:,Tindex,i))
              IF (FoundError(status, nf90_noerr, 2091, MyFile)) THEN
                IF (Master) THEN
...
!
     ...
     &                          GRID(ng) % umask,                       &
     &                          SEDBED(ng) % bedldu(:,:,i))
              IF (FoundError(status, nf90_noerr, 2205, MyFile)) THEN
                IF (Master) THEN
...
!
     ...
     &                          GRID(ng) % vmask,                       &
     &                          SEDBED(ng) % bedldv(:,:,i))
              IF (FoundError(status, nf90_noerr, 2257, MyFile)) THEN
                IF (Master) THEN
