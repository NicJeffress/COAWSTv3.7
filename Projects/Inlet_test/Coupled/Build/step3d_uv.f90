...
!
        ...
        DO k=1,N(ng)-1
          DO i=IstrU,Iend
            FCmax(i,k)=MAX(WK(i,k),0.0_r8)*cff/(on_u(i,j)*om_u(i,j))
            FCmin(i,k)=MIN(WK(i,k),0.0_r8)*cff/(on_u(i,j)*om_u(i,j))
          END DO
        END DO
        DO i=IstrU,Iend
          FCmax(i,0)=0.0_r8
...
!
          ...
          DO k=1,N(ng)-1
            DO i=Istr,Iend
              FCmax(i,k)=MAX(WK(i,k),0.0_r8)*cff/(on_v(i,j)*om_v(i,j))
              FCmin(i,k)=MIN(WK(i,k),0.0_r8)*cff/(on_v(i,j)*om_v(i,j))
            END DO
          END DO
          DO i=Istr,Iend
            FCmax(i,0)=0.0_r8
