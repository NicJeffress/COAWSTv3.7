...
!
     ...
     &                     GRID(ng) % h,                                &
     &                     SetFillVal = .FALSE.)
        IF (FoundError(status, nf90_noerr, 134, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                       GRID(ng) % umask,                          &
     &                       SEDBED(ng) % bedldu(:,:,i))
          IF (FoundError(status, nf90_noerr, 1637, MyFile)) THEN
            IF (Master) THEN
...
!
     ...
     &                       GRID(ng) % vmask,                          &
     &                       SEDBED(ng) % bedldv(:,:,i))
          IF (FoundError(status, nf90_noerr, 1660, MyFile)) THEN
            IF (Master) THEN
...
!
     ...
     &                       SEDBED(ng) % bed_frac(:,:,:,i))
          IF (FoundError(status, nf90_noerr, 1686, MyFile)) THEN
            IF (Master) THEN
...
!
     ...
     &                       GRID(ng) % rmask,                          &
     &                       SEDBED(ng) % bed(:,:,:,i))
          IF (FoundError(status, nf90_noerr, 1736, MyFile)) THEN
            IF (Master) THEN
