...
!
     ...
     &                        TRIM(STA(ng)%name))
          IF (FoundError(status, nf90_noerr, 530, MyFile)) THEN
...
!
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 24, MyFile)) RETURN
!
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 33, MyFile)) RETURN
!
      Vinfo( 1)='sg_zwbl'
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 40, MyFile)) RETURN
!
...
!
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 63, MyFile)) RETURN
!
...
!
     ...
     &                 1, (/stadim/), Aval, Vinfo, ncname,              &
     &                 SetParAccess = .FALSE.)
        IF (FoundError(exit_flag, NoError, 3136, MyFile)) RETURN
      END IF
